...
### BONUS using awk we can extract the gene name from the blastp output
## usful information can used as suplementary data for your annotated genome
awk -F '\t' 'BEGIN {OFS="\t"} {gsub(/sp\|[[:alnum:]]+\|/, "", $2); split($2, arr, "_"); print $1, arr[1], $3, $11}' rufa_vs_sprot_blastp_corrected_ids_95.tsv > arufa_vs_sprot_blastp_filtred_95_gene_names.tsv
### the gene_name can useful during the use of ShinyGO online web for enrichment gene
cut -f2 arufa_vs_sprot_blastp_filtred_95_gene_names.tsv > arufa_sprot_annotated_GeneName.txt
## 
awk -F '\t' 'BEGIN {OFS="\t"} \
NR==FNR {a[$1]=$0; next} \
($1 in a) {print $0, a[$1]}' rufa_vs_trembl_blastp_corrected_ids_95_3colum.tsv arufa_blastp_TrEMBL_filtred_95_gene_name.tsv > arufa_blastp_TrEMBL_filtred_95_gene_name_combined.tsv



: '
-F '\t': Sets the input field separator as a tab.
BEGIN {OFS="\t"}: Sets the output field separator as a tab.
gsub(/sp\|[[:alnum:]]+\|/, "", $2): Uses gsub to globally substitute the pattern sp|...| with an empty string in the second column.
split($2, arr, "_"): Splits the modified second column by underscores and stores it in an array arr.
print $1, arr[1], $3, $11: Prints the first, extracted gene name (first part after modifications), third, and eleventh columns separated by tabs.
This command removes the pattern "sp|...|" from the second column, then splits the remaining text by underscores and extracts the first part as the gene name. Adjust this command according to your specific file structure if needed.
'
