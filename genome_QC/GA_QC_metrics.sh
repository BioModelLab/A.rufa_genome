...
echo '#################################################'
echo 'Starting QUAST "Quality Assessment Tool for Genome Assemblies" '
echo '#################################################'

input=nexrdenovo.r1.fa
quast_out=nexrdenovo.r1_quast
quast -o $quast_out --fragmented $input -t $threads
conda deactivate

echo '#################################################'
echo 'Finished QUAST, The generated report saved to' 
pwd
echo '################################################# \n'
...
busco_out="nexrdenovo.r1_busco"
conda activate genomeasses
busco --offline -i $input -o $busco_out -m genome -c $threads -l /home/abdul/Desktop/busco_downloads/lineages/aves_odb10 
